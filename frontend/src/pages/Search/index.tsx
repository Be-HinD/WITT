...
const Search = () => {
	...
	const handleEnter = (event: React.KeyboardEvent<HTMLInputElement>) => {
		if (event.key === 'Enter' && input !== undefined) {
			localStorage.setItem(RecentItemType.KEYWORD, JSON.stringify([input, ...recentKeywords]))
		}
	}
	...
	return (
		<div className="text-whiteText">
			...
			<section className="flex flex-col gap-2 mt-14 px-4">
				...
				/>
				<fieldset>{ input ? <SearchResult /> : <RecentSearch recentKeywords={recentKeywords} />}</fieldset>
			</section>
		</div>
	)
}
